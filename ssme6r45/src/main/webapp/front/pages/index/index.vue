	...
	<view class="uni-padding-wrap">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.jiazhenggongsigongsimingcheng" type="text" placeholder="公司名称" ></input>
			</view>
			<view v-if="queryIndex==0" class="action">
				<button @tap="onPageTap('jiazhenggongsi')" :style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[2],borderColor:btnColor[2]}" class="cu-btn shadow-blur round">搜索</button>
			</view>
		</view>
			...
			<view class="right-content" @tap="onPageTap('jiazhenggongsi')" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				查看更多
			</view>
		</view>
		<view v-if='1 == 1 || 1 == 2' class="uni-product-list" :class="1 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('jiazhenggongsi',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in homejiazhenggongsilist" :key="index">
				                				                				<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}' class="uni-product-title">{{product.fuwumingcheng}}</view>
		...
		<view class="list" v-if='1 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('jiazhenggongsi',product.id)" v-for="(product,index) in homejiazhenggongsilist" :key="index" class="listm flex flex-between">
																																				<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.fuwumingcheng}}</view>
				</view>
	...
	export default {
		...
		async onShow() {
			...
			this.news = res.data.list
									...
									res = await this.$api.list('jiazhenggongsi', {
				page: 1,

				limit: 4
			});
			this.homejiazhenggongsilist = res.data.list
																							},
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.jiazhenggongsigongsimingcheng="";
			},
			//轮播图跳转
			onSwiperTap(e) {
			...
			onPageTap(tableName){
				if(this.queryIndex==0) {
					uni.setStorageSync('indexQueryCondition',this.searchForm.jiazhenggongsigongsimingcheng);
					this.searchForm.jiazhenggongsigongsimingcheng = '';
				}
