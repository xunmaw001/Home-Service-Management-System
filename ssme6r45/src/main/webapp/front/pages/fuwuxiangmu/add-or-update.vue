<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<input :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(40, 38, 38, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.fenlei" v-model="ruleForm.fenlei" placeholder="分类"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`fuwuxiangmu`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																																																if(this.ruleForm.id){
					await this.$api.update(`fuwuxiangmu`, this.ruleForm);
				}else{
					await this.$api.add(`fuwuxiangmu`, this.ruleForm);
				}
