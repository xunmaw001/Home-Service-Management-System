	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.gongsimingcheng" type="text" placeholder="公司名称" ></input>
			</view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(160, 156, 156, 0.82)"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
					...
					<text v-if="isAuth('jiazhenggongsi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('jiazhenggongsi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"40rpx","color":"#fff","borderWidth":"1","width":"80%","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('jiazhenggongsi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.gongsimingcheng="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.gongsimingcheng){
				...
				if(indexQueryCondition) {
					params['gongsimingcheng'] = '%' + indexQueryCondition + '%';
					uni.removeStorageSync('indexQueryCondition');
				}

				let res = await this.$api.list(`jiazhenggongsi`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiazhenggongsi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
												...
												if(this.searchForm.gongsimingcheng){
					searchForm['gongsimingcheng'] = '%' + this.searchForm.gongsimingcheng + '%'
				}	
																if(this.searchForm.fuwumingcheng){
					searchForm['fuwumingcheng'] = '%' + this.searchForm.fuwumingcheng + '%'
				}	
																																																																												let res = await this.$api.list(`jiazhenggongsi`, searchForm);
				// 如果是第一页数据置空
